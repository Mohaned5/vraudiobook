...
class PanimeDataset(PanoDataset):
    ...
    def get_data(self, idx):
        ...
        if self.mode == 'predict':
            ...
            FoV = np.array(cam_data['FoV'], dtype=np.float32).squeeze()
            theta = np.array(cam_data['theta'], dtype=np.float32).squeeze()
            phi = np.array(cam_data['phi'], dtype=np.float32).squeeze()
