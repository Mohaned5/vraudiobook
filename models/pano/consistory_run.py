...
# We use these latent resolutions for computing nearest–neighbor maps
LATENT_RESOLUTIONS = [32, 64]

def load_pipeline(gpu_id=0):
    float_type = torch.float16
    sd_id = "stabilityai/stable-diffusion-2-base"  # updated model ID for SD2.0
    
    device = torch.device(f'cuda:{gpu_id}') if torch.cuda.is_available() else torch.device('cpu')
    # Load the SD2.0 UNet from Diffusers:
    unet = UNet2DConditionModel.from_pretrained(sd_id, subfolder="unet", torch_dtype=float_type)
    scheduler = DDIMScheduler.from_pretrained(sd_id, subfolder="scheduler")
    
    # Load the custom extended attention pipeline (which should now be adapted for SD2.0)
    ...
    # Enable the FreeU mechanism (if you wish to use it) with your desired parameters.
...
def create_latents(story_pipeline, seed, batch_size, same_latent, device, float_type):
    # Sample noise using a given seed.
...
# Batch inference: Run generation over a batch of prompts.
def run_batch_generation(story_pipeline, prompts, concept_token,
    ...
    # Extended attention (anchor) pass.
    if perform_sdsa:
    ...
    print("Extended attn t_range:", extended_attn_kwargs['t_range'])
    ...
    # Extended attention (injection) pass.
    if perform_injection:
        ...
        out = story_pipeline(prompt=prompts, generator=g, latents=latents,
                             ...
                             num_inference_steps=n_steps)
        img_all = view_images([np.array(x) for x in out.images],
                              display_image=False, downscale_rate=downscale_rate)
        torch.cuda.empty_cache()
        gc.collect()
    else:
        img_all = view_images([np.array(x) for x in out.images],
...
# Anchors: Run generation with an anchor pass and then an injection pass.
