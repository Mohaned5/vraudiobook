# consistory_attention_processor.py
# (Copyright and license header remain unchanged)
...
def _extract_unet_part(name: str) -> str:
    # For SD2.0 UNet, assume the names include "down_blocks", "up_blocks", or "mid_block"
...
class ConsistoryAttnStoreProcessor:
    def __init__(self, attnstore, place_in_unet):
        ...
        # Use the helper function to extract the block type:
        self.curr_unet_part = _extract_unet_part(place_in_unet)
    ...
    def __call__(self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None, record_attention=True, **kwargs):
        ...
        # Linear projection and dropout remain the same
...
class ConsistoryExtendedAttnXFormersAttnProcessor:
    ...
    def __init__(self, place_in_unet, attnstore, extended_attn_kwargs, attention_op: Optional[Callable] = None):
        ...
        # For SD2.0, you may wish to limit the extension only to "up" blocks (or adjust as needed)
        self.extend_kv_unet_parts = extended_attn_kwargs.get('extend_kv_unet_parts', ['up'])
    ...
    def __call__(
        ...
        **kwargs
        ...
        if perform_extend_attn:
            ...
            if anchors_cache and anchors_cache.is_inject_mode():
                ...
                for i in range(batch_size):
                    ...
                    hidden_states = xformers.ops.memory_efficient_attention(
                        curr_q, curr_k, curr_v, op=self.attention_op, scale=attn.scale
                    )
        ...
        if feature_injector is not None:
            ...
            if anchors_cache and anchors_cache.is_inject_mode():
                hidden_states[batch_size // 2:] = feature_injector.inject_anchors(
                    hidden_states[batch_size // 2:], self.attnstore.curr_iter, output_res,
                    self.attnstore.extended_mapping, self.place_in_unet, anchors_cache
                )
            else:
                hidden_states[batch_size // 2:] = feature_injector.inject_outputs(
                    hidden_states[batch_size // 2:], self.attnstore.curr_iter, output_res,
                    self.attnstore.extended_mapping, self.place_in_unet, anchors_cache
                )
...
def register_extended_self_attn(unet, attnstore, extended_attn_kwargs):
    # (This DICT_PLACE_TO_RES is used only by registration code;
    # you may need to update it to reflect the resolutions for your SD2.0 UNet.)
